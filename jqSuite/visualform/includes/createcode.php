...
foreach( $rows as $row )
{
	...
	if( $events->item(0)->nodeValue != "__EMPTY_STRING_") {
		if(trim($events->item(0)->nodeValue) == "") {
			$ev = "{}";
		} else {
			$ev = trim($events->item(0)->nodeValue);
		}
...
if( $demo === true) {
	...
	//$str .= "}\n";
} else if(isset ($otherset['createconn']) && $otherset['createconn'] == 'no'){
	if(isset($otherset['conname']) && trim($otherset['conname']) != '') {
		$str .= $the_name."->setConnection( "."$".$otherset['conname']. ");\n";
	}
}
...
if( isset($otherset['formlayout']) && $otherset['formlayout'] != "" )
{
		...
		if(!( $otherset['formlayout'] === 'onecolumn' || $otherset['formlayout'] == 'twocolumn')) {
			$str .= $the_name."->setView('".$otherset['formlayout']."');\n";
		} else  {
			// compatibility
			$str .= $the_name."->setColumnLayout('".$otherset['formlayout']."');\n";
		}
...
if(!isset($otherset['formstyle'])) {
	$otherset['formstyle'] = 'jqueryui';
}
$str .= "// Set styling\n";
$str .= $the_name."->setStyling('".$otherset['formstyle']."');\n";
...
for($i=1; $i < count($mainarray); $i++ ) {
	...
	if($tmp['type'] == 'html') {
		//$str .= $the_name.'->addHtmlElement('".$tmp['name']."','".$tmp['type']."', array(";
		//var_dump($tmp['prop']);
		$str .= $the_name.'->addHtmlElement("'.$tmp['name'].'","'. htmlspecialchars_decode($tmp['prop']['html']).'");'."\n";
	} else {
	if($tmp['type'] == 'group') {
...
if( trim($otherset['ajax_data']) ) {
	...
	$new_array = array_map(function($key, $value){return "\"". $key."\" => \"".$value."\" "; }, array_keys($apr), array_values($apr));
