...
<script type="text/javascript" src="../js/trirand/src/jquery.jqGrid.js"></script>
<script type="text/javascript" src="../js/trirand/jquery.jqForm.js"></script>
...
//menus
	...
	jQuery('#tabs','.ui-layout-east').tabs({
        spinner: 'Loading',
	...
	*/
	...
	$("#context").contextmenu({
		position : function(event, ui) {
			return {my: "left+35 top-23", at: "left bottom", of: ui.target};
		},
		menu: [
			{title: "Image", cmd: "eimage", uiIcon: "ui-icon-image"},
			{title: "Date", cmd: "edate", uiIcon: "ui-icon-calendar"},
			{title: "Number", cmd: "enumber", uiIcon: "ui-icon-calculator"},
			{title: "Range", cmd: "erange", uiIcon: "ui-icon-arrow-2-e-w"},
			{title: "Email", cmd: "eemail", uiIcon: "ui-icon-mail-closed"},
			{title: "Url", cmd: "eurl", uiIcon: "ui-icon-link"},
			{title: "Search", cmd: "esearch", uiIcon: "ui-icon-search"},
			{title: "Telephone", cmd: "etel", uiIcon: "ui-icon-volume-off"},
			{title: "Color", cmd: "ecolor", uiIcon: "ui-icon-bookmark"}
        ],
	...
	$( "#ehtml" ).button({
		...
		$.jqform.addTreeElement("html", "ui-icon-pencil");
	});
	$( "#ehidden" ).button({
	...
	$( "#emore" ).button({
		...
		$("#context").contextmenu("open", $(".hasmenu:first"), {foo: "bar"});
	});
	$(document).on('click', "#showglob", function() {
	...
	var trevent=null;
	$(document).on('click', ".editcode", function(e){
...
//
	...
	$("#dialogsave").dialog({
			...
			open : function (ui,event) {
				$("#dir_name_tosave").val(workingdir);
			},
			buttons: {
				Save : function (){
					//if(saveallowed ) {
    ...
    $('#savelist').fileTree({root: workingdir, "script":"includes/jqueryFileTree.php" }, function(file, filetype) {
		if(filetype=='file') {
			var dir = $.trim($("#dir_name_tosave").val());
			var fl = dir.length;
			if(fl > -1 ) {
				saveallowed = true;
				var fn = file.substring(fl);
				$("#f_name_tosave").val( fn );
			}
		} else {
			saveallowed = false;
		...
		buttons: [
			{
				...
				click : function()
				{
					...
					var u7 = $("#formstyle").val();
					jQuery("#west-grid").jqGrid('setGridParam',{userData:{formlayout:u0, formheader:u1, formicon:u2, formfooter:u3, tablestyle:u4, labelstyle:u5, datastyle:u6, formstyle:u7}});
					$( this ).dialog( "close" );
				}
			},
			{
...
<body>
    <div class="ui-layout-north ui-helper-reset ui-helper-clearfix ui-widget" >
		...
		<?php if(!$demo) {
			echo '<button  id="bphpscript">PHP Script</button>';
		}
		?>
    ...
    <div class="ui-layout-east ui-widget ui-widget-content">
		...
		<div id="dialogset" title="Form Settings">
			<table style="width:100%;table-layout: fixed;"> 
				...
				<tr>
					...
					<td>
						<select name="formlayout" id="formlayout" class="ui-widget-content ui-corner-all select-ui">
							...
							<option value="twocolumn">Two Column (table)</option>
							<option value="onecolumn">One Column (table)</option>
							<option value="nostyle">No Styling</option>
						</select>
					</td>
				...
				<tr>
					<td>
						<label for="formstyle"> Form Styling:</label>
					</td>
					<td>
						<select name="formstyle" id="formstyle" class="ui-widget-content ui-corner-all select-ui">
							<option value="jqueryui">jQuery UI</option>
							<option value="bootstrap">Bootstrap</option>
						</select>
					</td>
					<td class="help" title="<?php echo $formstyle?>"><span class='ui-icon ui-icon-info'></span></td>
				</tr>
				<tr>
					<td>
    ...
    <div class="ui-layout-west ui-widget ui-widget-content">
		...
		<button  id="ehtml" style="width:31px;">Add HTML element</button><br/>
		<button  id="emore" style="width:31px;">Add more elements</button><br/>
		<div id="context"></div>		
    </div>
</body>
</html>
