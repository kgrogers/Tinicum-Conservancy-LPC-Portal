...
class jqScheduler {
	...
	public $localenpath = "Scheduler/localization";
	public $templatepath = "Scheduler/templates";
	public $databasepath = "Scheduler/backend";
	 ...
	 */
	public function sendErrorHeader ($errorMsg="") {
		if($errorMsg) {
	 ...
	 * 
	 * @param type $a array
	 * Add additional data, that can be inserted or/and updated
	 * when a edit or insert is activeted - see editeven, newEvent opers
	 */
	public function setUserData ($a = array())
	{
		if(is_array($a)) {
			$this->userdata = $a;
		}
	...
	*/
	private function exportiCal($start, $end) 
	{
		if (!empty($this->user_id)) {
			...
			if($search == 'true') {
				$searchconds = $this->composeSearch($stype, $this->backend->getDBMap());						
	...
	private function htmlTable()
	{
		...
		try {
			...
			foreach ($rowdata as $data) {
				...
				if($data['allDay'] == 1) 
				{
					$ald = "<td colspan=\"2\" align=\"center\" style=\"border: 1px soid #5c9ccc;\">".$locale->form_allDay."</td>";
				} else {
			...
			$layout->set('h_className', $locale->form_className);
	...
	protected function getDBName ($jsname, $map)
	{
		if(strlen(trim($jsname))>0) {
			$key = array_search($jsname, $map);
			if($key !== false) {
				return $key;
			}
		}
	 ...
	 */
	protected function getStringForGroup( $group, $prm, $dbmap )
	{
		$datearray = array($this->getDBName('start', $dbmap), $this->getDBName('end',$dbmap));
		$i_ = $this->I;
		...
		if( isset ($group['groups']) && is_array($group['groups']) && count($group['groups']) >0 )
		{
			for($j=0; $j<count($group['groups']);$j++ )
			{
				...
				try {
					$dat = $this->getStringForGroup($group['groups'][$j], $prm, $dbmap);
					$s .= $dat[0];
					$prm = $prm + $dat[1];
				} catch (Exception $e) {
					echo $e->getMessage();
		...
		if (isset($group['rules']) && count($group['rules'])>0 ) {
			try{
				$integers = array($this->getDBName('user_id',$dbmap), $this->getDBName('start',$dbmap), $this->getDBName('end',$dbmap),$this->getDBName('allDay',$dbmap));
				foreach($group['rules'] as $key=>$val) {
					if (strlen($s) > 1) {
						$s .= " ".$group['groupOp']." ";
					}
					$field = $this->getDBName($val['field'],$dbmap);
					$op = $val['op'];
					$v = $val['data'];
	 ...
	 * @return array
	 */
	protected function _buildSearch( array $prm=null, $str_filter = '', $dbmaps )
	{
		...
		if($jsona) {
			if($rules && count($rules) > 0 ) {
				...
				$ret = $this->getStringForGroup($jsona, $prm, $dbmaps);
	 ...
	 */
	protected function composeSearch( $stype='simple', $dbmaps)
	{
		$s = "";
		$prm = array();
		$q = $this->quote;
		if($stype == 'simple') {
			$title = $this->getDBName('title',$dbmaps);
			$location = $this->getDBName('location',$dbmaps);
			$description = $this->getDBName('description',$dbmaps);
			$i_ = $this->I;
			...
			$res = $this->_buildSearch(null, '', $dbmaps);
			$s = $res[0]; $prm = $res[1];
		}
		return array($s, $prm);
	}
	...
	public function setSelectOptions($templateid, $data) {
		$this->selects[$templateid] = $data;
	}
	protected function buildSelect ( $template) {
		if(count($this->selects) > 0) {
			foreach($this->selects as $key=>$val) {
				$s ='';
				if(is_string($val)) {
					$aset = jqGridDB::query($this->db,$val);
					while($row = jqGridDB::fetch_num($aset, $this->db)) {
	...
	public function render($script=true, $echo = true) 
	{
		...
		if($this->oper && in_array($this->oper, array('getEvent','editEvent','removeEvent','newEvent','resizeEvent', 'moveEvent', 'switchUser', 'exportEvents','printEvents'))) {
			if($this->backend_type == 'database') {
				require_once($this->databasepath.'/database.php');
				...
				switch($this->oper) {
					case  'getEvent':
						...
						if(is_array($data)) {
							...
							$this->sendErrorHeader( $this->backend->getError() );
						}
						break;
					case 'editEvent' :
						//$data = jqGridUtils::Strip($_POST);
						$data = jqGridUtils::array_extend(jqGridUtils::Strip($_POST), $this->userdata);
						if( strtolower($this->encoding) != 'utf-8' ) {
							foreach ( $data as $k=>$v)
							{
								$v = iconv("utf-8", $this->encoding."//TRANSLIT", $v);
						...
						if(function_exists($this->customFunc)) {
							$result = call_user_func($this->customFunc,'editEvent',$data);
						}
						if($result) {
							if(!$this->backend->editEvent( $data )) {
								$this->sendErrorHeader( $this->backend->getError() );
							} else {
						...
						break;
					case 'removeEvent':
						$ev_id = jqGridUtils::GetParam('event_id','0');
						$result = true;
						if(function_exists($this->customFunc)) {
							$result = call_user_func($this->customFunc,'removeEvent',array('event_id'=>$ev_id));
						}
						if($result) {
							if(!$this->backend->removeEvent( $ev_id )) {
								$this->sendErrorHeader( $this->backend->getError() );
							}
						}
						break;
					case 'newEvent':
						...
						$result = true;
						if(function_exists($this->customFunc)) {
							$result = call_user_func($this->customFunc,'newEvent', $data);
						}
						if($result) {
					...
					case 'resizeEvent':
						$result = true;
						$eid = jqGridUtils::GetParam('event_id','');
						$st = jqGridUtils::GetParam('start','');
						$en =  jqGridUtils::GetParam('end','');
						if(function_exists($this->customFunc)) {
							$result = call_user_func($this->customFunc,'resizeEvent', array('event_id'=>$eid, 'start'=>$st, 'end'=>$en));
						}
						if($result) {
							if(!$this->backend->resizeEvent($eid, $st, $en)) {
								$this->sendErrorHeader( $this->backend->getError() );
							}
						}
						break;
					case 'moveEvent':
						...
						$st = jqGridUtils::GetParam('start','');
						$en =  jqGridUtils::GetParam('end','');
						$ald = jqGridUtils::GetParam('allDay','0');
						if(function_exists($this->customFunc)) {
							$result = call_user_func($this->customFunc,'moveEvent', array('event_id'=>$eid, 'start'=>$st, 'end'=>$en, 'allDay'=>$ald));
						}
						if($result) {
							if(!$this->backend->moveEvent($eid, $st , $en ,$ald )) {
								$this->sendErrorHeader( $this->backend->getError() );
							}
						}
						break;
			...
			try {
				...
				$tdata->set('className', $locale->form_className);
				...
				$tdata->set('allDay', $locale->form_allDay);
				...
				// finally custom selects
				$this->buildSelect($tdata);
				// render it
				$st = $tdata->output();
				$tdata = null;
