...
class jqAutocomplete
{
	 ...
	 *
	 * @var string
	 * the zindex of the element. default 1000
	 */
	public $zIndex = '1000';
	...
	/**
	 * Instruct the autocomplete to return data in a JSONP way
	 * @var boolean
	 */
	public $JSONP = false;
	/**
		...
		if(strlen($this->SelectCommand) > 0 ) {
			...
			if($this->itemLength > 0 && !$this->loadAll) {
				if($this->dbtype=="adodb") {
					$sql1 = jqGridDB::limit($this->conn, $this->SelectCommand, $this->itemLength, 0, $params );
					$adodblimit = true;
				} else {
					$order = '';
					if($this->dbtype == "sqlsrv" || $this->dbtype == "odbcsqlsrv") {
						if(preg_match('/\s+ORDER\s+BY\s+/is',$this->SelectCommand, $matches))
						{
							$pos = strpos(strtoupper($this->SelectCommand), $matches[0]);
							if($pos !== false) {
								$pos2 = $pos + strlen($matches[0]);
								$order = trim(substr($this->SelectCommand,$pos2));
								$this->SelectCommand = trim(substr($this->SelectCommand,0, $pos));
							}
						}
					}
					$sqlCmd = jqGridDB::limit($this->SelectCommand, $this->dbtype, $this->itemLength, 0, $order );
				}
			} else {
			...
			if( $this->dbtype == 'oci8' || $this->dbtype == 'db2' || $this->dbtype == 'sqlsrv' || $this->dbtype == 'odbcsqlsrv') { //Oracle and DB2 has 1 col more
				$ncols--;
			}
...
function (event, ui)
{
	...
	if(test.length > 1 ) {
		reqid = test[0];
		reqid = "#"+reqid+ "_" + "$target".replace("#","");
	}
	if(ival) {
		jQuery(reqid).val(ival);
	} else {
		jQuery(reqid).val("");
...
TARGET;
			...
			$s .= "jQuery('".$element."').autocomplete('widget').css('z-index','".$this->zIndex."');";
			if($this->scroll) {
			...
			if(trim($this->element) === trim($element) ) {
				...
				$callback = jqGridUtils::GetParam("callback");
				if(function_exists('json_encode') && strtolower($this->encoding) == 'utf-8') {
					echo $callback ? $callback."(". json_encode($this->getACData()). ")": json_encode($this->getACData());
				} else {
					echo $callback ? $callback."(". jqGridUtils::encode($this->getACData()).")" : jqGridUtils::encode($this->getACData());
				}
			}
		}
	}
