...
class jqGridUtils
{
	...
	public static function parseDate($patternFrom,$date, $patternTo=''){
		if (version_compare(PHP_VERSION, '5.3.0') >= 0)  {
			$dateobj = DateTime::createFromFormat($patternFrom, $date);
			if($patternTo) {
				return $dateobj->format($patternTo);
			} else {
				return (int)$dateobj->format("U");
			}
		} else {		
 ...
 */
	...
	public static function phpToExcelDate ($phpdate)
	{
		$PHPFormatOptions = array('A', 'y', 'Y', 'M', 'F', 'i', 'm', 'n', 'l', 'D', 'd', 'j', 's');
		$ExcelOptions = array('am/pm', 'yy', 'yyyy', 'mmm', 'mmmm', 'MM', 'mm', 'm','dddd', 'ddd', 'dd','d','ss'); // and so on
		return str_replace($PHPFormatOptions, $ExcelOptions, $phpdate);
	}
	/**
