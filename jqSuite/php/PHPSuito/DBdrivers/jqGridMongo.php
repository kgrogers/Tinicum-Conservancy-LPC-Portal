...
class jqGridDB
{
	...
	public static function getColumnMeta($index, $sql)
	{
		if($sql && $index >= 0)
		{
			...
			$newmeta["len"]  = is_array($values[$index]) ? count($values[$index]) : strlen($values[$index]);
			return $newmeta;
		}
	}
	/**
	...
	public static function _mongocount($collection, $query, $sumcols)
	{
		...
		$reduce = "function (obj, prev) { prev.COUNTR++;".$s."}";
		$res = $collection->group($keys, $initial, $reduce, array("condition"=>$query));

		return isset($res['retval'][0]) ? $res['retval'][0] : $res['retval'];
	}
