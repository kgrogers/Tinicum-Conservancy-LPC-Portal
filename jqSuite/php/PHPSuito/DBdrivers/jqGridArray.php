...
class jqGridArrayPreg 
{
	/*
	-------------------------------------------- */
	public static function parse_where_key($key)
	{
		if ( preg_match('/\./', $key) )
		{
...
class jqGridArray
{
	...
	private function parse_query()
	{
		...
		$this->parse_query= preg_replace_callback(
			'#ORDER(\s){2,}BY(\s+)(.*)(\s+)(ASC|DESC)#i', 
			function($m) { return 'ORDER BY '.$m[3].' '.$m[5]; },
			$this->query);
		if(strpos($this->query, 'AS COUNTR') !== false) {
			$this->parse_query			= str_replace('AS COUNTR', '', $this->parse_query);
			$this->countr =  TRUE;
		} else {
...
/*		
		...
		$replacements[]	= "'trim(\\1)'";
*/		
		$patterns_and_callbacks['#LOWER\((.*)\)#i'] = function($m) { return strtolower($m[1]);};
		$patterns_and_callbacks['#UPPER\((.*)\)#i'] = function($m) { return strtoupper($m[1]);};
		$patterns_and_callbacks['#TRIM\((.*)\)#i'] = function($m) { return ($m[1]);};
...
/*		
		...
		$replacements[]	= "'\\1'.\$this->parse_where_key(\"\\4\").'\\5 == ('.\$this->parse_in(\"\\10\").') '";
*/
		$patterns_and_callbacks['#(([a-zA-Z0-9\._]+)(\())?([a-zA-Z0-9\._]+)(\))?(\s)+(=|IS)(\s)+([[:digit:]]+)(\s)*#i'] = 
			function($m) { return $m[1].jqGridArrayPreg::parse_where_key( $m[4] ).$m[5]." == ".$m[9];};
		$patterns_and_callbacks['#(([a-zA-Z0-9\._]+)(\())?([a-zA-Z0-9\._]+)(\))?(\s)+(=|IS)(\s)+(\'|\")(.*)(\'|\")(\s)*#i'] = 
			function($m) { return $m[1].jqGridArrayPreg::parse_where_key( $m[4] ).$m[5]." == '".$m[10]."'";};
		$patterns_and_callbacks['#(([a-zA-Z0-9\._]+)(\())?([a-zA-Z0-9\._]+)(\))?(\s)+(>|<)(\s)+([[:digit:]]+)(\s)*#i'] = 
			function($m) { return $m[1].jqGridArrayPreg::parse_where_key( $m[4] ).$m[5]." ".$m[7]." ".$m[9];};
		$patterns_and_callbacks['#(([a-zA-Z0-9\._]+)(\())?([a-zA-Z0-9\._]+)(\))?(\s)+(>|<)(\s)+(\'|\")(.*)(\'|\")(\s)*#i'] = 
			function($m) { return $m[1].jqGridArrayPreg::parse_where_key( $m[4] ).$m[5]." ".$m[7]." '".$m[10]."'";};

		$patterns_and_callbacks['#(([a-zA-Z0-9\._]+)(\())?([a-zA-Z0-9\._]+)(\))?(\s)+(<=|>=)(\s)+([[:digit:]]+)(\s)*#i'] = 
			function($m) { return $m[1].jqGridArrayPreg::parse_where_key( $m[4] ).$m[5]." ".$m[7]." ".$m[9];};
		$patterns_and_callbacks['#(([a-zA-Z0-9\._]+)(\())?([a-zA-Z0-9\._]+)(\))?(\s)+(<=|>=)(\s)+(\'|\")(.*)(\'|\")(\s)*#i'] = 
			function($m) { return $m[1].jqGridArrayPreg::parse_where_key( $m[4] ).$m[5]." ".$m[7]." '".$m[10]."'";};
		$patterns_and_callbacks['#(([a-zA-Z0-9\._]+)(\())?([a-zA-Z0-9\._]+)(\))?(\s)+(<>|IS NOT|!=)(\s)+([[:digit:]]+)(\s)*#i'] = 
			function($m) { return $m[1].jqGridArrayPreg::parse_where_key( $m[4] ).$m[5]." != ".$m[9];};
		$patterns_and_callbacks['#(([a-zA-Z0-9\._]+)(\())?([a-zA-Z0-9\._]+)(\))?(\s)+(<>|IS NOT|!=)(\s)+(\'|\")(.*)(\'|\")(\s)*#i'] = 
			function($m) { return $m[1].jqGridArrayPreg::parse_where_key( $m[4] ).$m[5]." != '".$m[10]."'";};
		$patterns_and_callbacks['#(([a-zA-Z0-9\._]+)(\())?([a-zA-Z0-9\._]+)(\))?(\s)+(IS)?(NOT IN)(\s)+\((.*)\)#i'] = 
			function($m) { return $m[1].jqGridArrayPreg::parse_where_key( $m[4] ).$m[5]." != ( ". $this->parse_in($m[10])." )";};
		$patterns_and_callbacks['#(([a-zA-Z0-9\._]+)(\())?([a-zA-Z0-9\._]+)(\))?(\s)+(IS)?(IN)(\s)+\((.*)\)#i'] = 
			function($m) { return $m[1].jqGridArrayPreg::parse_where_key( $m[4] ).$m[5]." == ( ". $this->parse_in($m[10])." )";};
...
/*
		...
		//$replacements[]	= "'\$this->isLike(\"\\5\", '.\$this->parse_where_key(\"\\1\").')'";
*/
		$patterns_and_callbacks['#([a-zA-Z0-9\._]+)(\s)+NOT LIKE(\s)*(\'|\")(.*)(\'|\")#i'] =
			function($m) { 
				$dd = jqGridArrayPreg::parse_where_key($m[1]);
			return "!\$this->isLike('".$m[5]."', $dd)";
			};
		$patterns_and_callbacks['#([a-zA-Z0-9\._]+)(\s)+LIKE(\s)*(\'|\")(.*)(\'|\")#i'] =
			function($m){ 
				$dd = jqGridArrayPreg::parse_where_key($m[1]);
				return "\$this->isLike('".$m[5]."', $dd)";
			};
		...
		foreach($wdata as $k=>$wh) {
			...
			foreach($wdataor as $kk=>$whor) {
				...
				$wdataor[$kk] = stripslashes(trim(preg_replace_callback_array($patterns_and_callbacks, $whor)));
			}
		...
		foreach ($this->tables as $table)
		{
			foreach ($table as $row)
			{
				...
				if($this->count_query==TRUE) {
					if($this->countr) {
						$this->response = array("COUNTR"=>$rcount);
					} else {
	...
	public static function fetch_object( $psql, $fetchall, $conn=null )
	{
		if($psql) {
			if(!$fetchall)
			{
				...
				{
					$ret = (object)$psql;
				} else if( isset($psql[self::$acnt]) ) {
					$ret =  (object)$psql[self::$acnt];
				}
				self::$acnt++;
				return $ret;
