...
function(rowid, value, name) {
	...
	if(srow) {
		var gridrow = jQuery("#grid").jqGrid('getRowData',srow);
		return "select.php?id_emp="+gridrow.EmployeeID;
	}
}
SELECT;
...
$grid->setColProperty("EmployeeID",array("edittype"=>"select", "hidden"=>false,"width"=>20,"editoptions"=>array("dataUrl"=>"js:".$select)));
