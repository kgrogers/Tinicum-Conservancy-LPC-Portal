...
{	 
	...
	*/
	...
	searchOpers : [],
	/**
	 * default fields in calendar. Should not be changed
	 */
	default_fields : ['title', 'allDay', 'location', 'className', 'access', 'start', 'starttime', 'end','endtime', 'description', 'user_id'],
	_all_fields : [],
	/**
	 * holds the diffrence in fields in case of custom fields
	 */
	diff : [],
	diff_type : [],
	...
	customError : function (errMsg) {
		$('body').append("<div id='error-message' title='Erorr'><p>"+errMsg+"</p></div>");
	...
	/**
	 * Set the custom edit fields if they defined in database and template
	 * 
	 * @param {jquery object} dialogContent the content of the dialog
	 * @param {object} event scheduler object event
	 * @returns none
	 * 
	 */
	setCustomEditFields : function (dialogContent, event) {
		$.jqschedule.diff =[];
		$.jqschedule.diff_type=[];
		$.each($("form", dialogContent)[0].elements, function(ind, el) {
			if( $.inArray(el.id, $.jqschedule.default_fields ) === -1) {
				if(event.hasOwnProperty(el.id)) {
					$.jqschedule.diff.push(el.id);
					$.jqschedule.diff_type.push( el.type );
					if( el.type === 'checkbox') {
						if( event[el.id] == "1") {
						...
						var ovm = event[el.id].split(","), k=0, size = ovm.length, opt = $("#" + el.id + ' option');
						ovm = $.map(ovm,function(n){return $.trim(n);});
						for(k; k < size; k++){
							// filter the options with the specific value and select them
							opt.filter('[value="'+ovm[k]+'"]').prop('selected', true);
						}
					} else {
						$("#" + el.id).val( event[el.id] );
					}
				}
			}
		});	
	...
	eventEditDialog : function(event, jsEvent, view  )
	{
		...
		$('#allDay',dialogContent).attr('checked',event.allDay);
		...
		$.jqschedule.setCustomEditFields(dialogContent, event);
		...
		buttons[$.jqschedule.locales.bSave] = function() {
			...
			event.end=en;
			event._end = en;
			if($("#allDay", dialogContent).is(':checked')) {
				event.allDay = true;
			} else {
				event.allDay = false;
			}
			...
			dp.oper = 'editEvent';
			$.each($.jqschedule.diff, function(i,el) {
				var val;
				if($.jqschedule.diff_type[i] === 'checkbox') {
					val =  $('#'+el,dialogContent).is(":checked") ? "1" : "0";
				} else if ($.jqschedule.diff_type[i] === 'select-multiple') {
					val =	$('#'+el,dialogContent).val().join();
				} else {
					val = $('#'+el,dialogContent).val();
				}
				event[el] = dp[el] = val;
			});
			// send it to server
			$.ajax({
				...
				success : function(res) {
					...
					if(res.hasOwnProperty('error') & res.error !== "") {
						$.jqschedule.customError( res.errorMsg || 'Error');
					} else  {
				...
				error : function(xhr,st,err) {
					if($.isFunction($.jqschedule.loadError)) { 
						$.jqschedule.loadError(xhr,st,err); 
					}
				}
			});
			// update 
	...
	eventAddDialog : function(date, allDay, jsEvent, view)
	{
		...
		$('#allDay',dialogContent).attr('checked',allDay);
		// FireFox 4 fix
	...
	_filSearchTable : function( result, tblid )
	{
		...
		if(result && result.length) {
			...
			var tdata = "", sep = "~`", i=0;
			$.jqschedule._all_fields = [];
			$.each(result, function() {
				if(i===0) {
					$.jqschedule._all_fields = $.map(this,function(val,key ){
						return key;
					});
				}
				...
				tdata += $.map(this,function(val, key ){return val;}).join( sep ); 
						//this.id+sep+ this.title+sep+this.start+sep+this.end+sep+this.description+sep+this.location+sep+this.className+sep+this.access+sep+this.allDay+sep+this.user_id;
				tdata += "</td>";
				tdata += "</tr>";
				i++;
			});
	...
	editFromSearch : function ( eventid )
	{
		...
		var event = {};
		$.each($.jqschedule._all_fields, function(i,n){
			if(n === 'start' || n === 'end') { data[i] = new Date( parseFloat(data[i])*1000 ); }
			if(n === 'allDay') { data[i] = data[i] == '1' ? true : false;}
			event[n] = data[i];
		});
	...
	processScript :function() 
	{
		...
		$('#allDay',$.jqschedule.eventid).click(function(){
			if(this.checked) {
